...
def _to_en_digits(s: str) -> str:
    """تبدیل اعداد فارسی/عربی به انگلیسی + علائم نگارشی فارسی (، ٫)."""
    s = s.translate(str.maketrans("۰۱۲۳۴۵۶۷۸۹٠١٢٣٤٥٦٧٨٩", "01234567890123456789"))
    return s.replace("٬", ",").replace("،", ",").replace("٫", ".")


# نرمال‌سازی کلیدهای فارسی: اعداد فارسی/کامای فارسی داخل کلیدها رو به انگلیسی تبدیل کن
# (کاربر «طلای ۱۸» رو می‌نویشه، parse_input اول اعداد رو انگلیسی می‌کنه — کلیدها هم هم‌فرم باشن)
_NORMALIZED_FA = {}
for _k, _v in list(FA_TO_KEY.items()):
    _NORMALIZED_FA[_to_en_digits(_k)] = _v
FA_TO_KEY.update(_NORMALIZED_FA)
...
def parse_input(text: str) -> Tuple[Optional[str], Any]:
    ...
    # کلید کاملِ فارسی با اعداد انگلیسی (مثل «طلای ۱۸» → «طلای 18»)
    key = FA_TO_KEY.get(tc_clean)
    if key:
        return ("single", key)
    ...
    if nums:
        ...
        if amount is not None:
            key = FA_TO_KEY.get(tc_clean) or _resolve_text(text_no_nums_clean or text_no_nums)
            if key:
                return ("calc", (key, amount))

    # فقط متن
    key = _resolve_text(tc)
    if key:
        return ("single", key)
