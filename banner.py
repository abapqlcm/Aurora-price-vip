...
def render_banner_video(code: str, duration: float = 2.0, fps: int = 16) -> Optional[bytes]:
    ...
    with _VIDEO_LOCK:
        ...
        try:
            ...
            if out is None:
                log.warning("video render returned None %s (hist=%s)", code, len((data or {}).get("history") or []))
            if out and data:
...
def _render_video_uncached(code: str, ck: str, now: float, duration: float, fps: int, prefetched_data: dict | None = None) -> Optional[bytes]:
    ...
    # فالبک تاریخچه: TGJU گاهی 500 می‌ده → اول کندل‌های واقعی والکس، بعد snapshotهای محلی
    if len(hist) < 4:
        wh = datafeeds.wallex_history("USDTTMN", 24)
        if len(wh) >= 4:
            hist = wh
        else:
            lh = local_history(ck)
            if len(lh) >= 2:
                ...
                hist = [data["price"] * 0.999, data["price"]]
    if len(hist) < 2:
        return None
    ...
    # فالبک دوتایی/تخت: بازه رو ±0.6% باز کن تا خط به کف نچسبه و موج دیده بشه
    if mx - mn < mx * 0.004:
        mid = (mn + mx) / 2
        mn, mx = mid * 0.994, mid * 1.006
    if mx == mn:
        mx = mn * 1.001 + 1
    rng = mx - mn
    ...
    try:
        ...
        try:
            ...
            for f in range(n_frames):
                ...
                try:
                    ...
                    _ring_r = lr + 10 + int(26 * (0.5 + 0.5 * math.sin(_pulse)))
                    _ring_a = int(120 * (0.5 - 0.5 * math.sin(_pulse)))
                    if _ring_a > 4:
                ...
                if len(pts) >= 2:
                    ...
                    mask = ImageChops.darker(mask, _CHART_CLIP_MASK)
                    grad = Image.new("RGBA", (w, h), (0, 0, 0, 0))
        ...
        try:
            ...
            log.warning("video ffmpeg wait %s: %s", code, e_w)
            proc.kill()
            proc.wait()
            return None
    ...
    if not out or len(out) < 1000:
        log.warning("video output empty %s: %d bytes", code, len(out) if out else 0)
        return None
